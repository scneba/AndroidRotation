 ...
 * Functions originate from stackoverflow
 */
...
public class HttpRequest {
     ...
     *
     * @param method Method method of HTTP request
     * @throws IOException - should be checked by caller
     */
    public HttpRequest prepare(Method method,int connectionTimeout,int readTimeOut)  throws IOException {
    ...
    public int send() throws IOException {
        return urlConnection.getResponseCode(); //return HTTP status code to indicate whether it successfully sent
    }
     ...
     * @throws IOException - should be checked by caller
     */
    public String sendAndReadString() throws SocketException,SocketTimeoutException,IOException {
        InputStream is =new BufferedInputStream(urlConnection.getInputStream());

        //handle url redirectsn
        if (!url.getHost().equals(urlConnection.getURL().getHost())) {
            throw new IOException("url redirect was done");
        }
